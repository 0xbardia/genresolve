  ...
  return (
    <div className="flex items-center gap-1.5 sm:gap-2">
      {!isOnCorrectNetwork && (
        <button
          type="button"
          className="btn btn-sm border border-[rgba(230,192,105,0.35)] bg-[var(--warning-bg)] text-[var(--gold)]"
          disabled={busy}
      ...
      <div className="wallet-pill" title={address ?? undefined}>
        <span className="wallet-pill-avatar" aria-hidden>
          {address ? address.slice(2, 4).toUpperCase() : "??"}
        </span>
        <span className="hidden sm:inline">{shortAddress(address)}</span>
        {isOnCorrectNetwork && (
          <span className="live-dot hidden sm:inline-block" title="On network" />
        )}
      </div>
      <button
        type="button"
